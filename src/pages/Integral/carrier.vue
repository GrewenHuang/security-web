  ...
  >
    ...
    <div class="box-title">
      ...
      <div class="name-gz">
       ...
       <div class="kpi-box">
      <div class="box-table">
        <el-table :data="tableData" style="width: 100%" border max-height="440" :header-cell-style="{background:'#f8f8f8',color:'#181b3b'}">
          <el-table-column prop="DepartmentName" label="部门" ></el-table-column>
          <el-table-column prop="Project" label="考核项目"></el-table-column>
          <!-- <el-table-column prop="Project" label="考核项目" width="50"></el-table-column> -->
    ...
    >
      <el-scrollbar class="scrollbar" ref="scrollbar">
        <el-form ref="form" label-position="top" :model="info" :rules="rules">
          ...
          <el-form-item label="承运商名称" prop="Project">
...
export default {
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          this.loading = true;
          this.Post("/SaveCustomerAssessmentVendor", params).then(result => {
    ...
    saveKpi(data) {
      ...
      // 分数保存
      // if (this.infoSet.isAdd == 1) {
      //   this.resetArr();
      //   this.MonthList[this.infoSet.index].Score = this.infoSet.Score;
      //   this.infoSet.ScoreJsonList = this.MonthList;
      //   this.infoSet.ScoreJson = JSON.stringify(this.MonthList);
      // } else {
      //   this.list[this.infoSet.id].ScoreJsonList[
      //     this.infoSet.index
      //   ].Score = this.infoSet.Score;
      //   this.list[this.infoSet.id].ScoreJsonList[
      //     this.infoSet.index
      //   ].Incentive = this.infoSet.Incentive;
      //   this.infoSet.ScoreJsonList = this.list[this.infoSet.id].ScoreJsonList;
      //   this.infoSet.ScoreJson = JSON.stringify(
      //     this.list[this.infoSet.id].ScoreJsonList
      //   );
      // }
      ...
      // this.dislogShowKh = false;
      //       // 合并ScoreJsonList 字段 月份考核
      // let list = JSON.parse(JSON.stringify(this.infoSet));
      // let list1 = [];
      // if (list.ScoreJsonList && list.ScoreJsonList.length > 0) {
      //   let obj = {}; //合并
      //   list.ScoreJsonList.forEach((item, index) => {
      //     if (index % 2 == 0) {
      //       if (!obj[item.Month]) {
      //         obj[item.Month] = item;
      //       } else {
      //         obj[item.Month].Score = item.Score;
      //       }
      //     } else {
      //       if (!obj[item.Month]) {
      //         obj[item.Month] = item;
      ...
      //   list.ScoreJsonList = list1;
      //   list.ScoreJson = JSON.stringify(list1);
      // }
      // this.$refs.formKH.validate(valid => {
      //   if (valid) {
      console.log(data2,'传递数')
          this.loading = true;
          this.Post("/SaveCustomerAssessmentVendorDetail",data2 ).then(
            result => {
              if (!result || !result.status) {
                this.$message({
                  message: "请求异常",
    ...
    async getList() {
      ...
      this.loading = true;
      try {
        let res = await this.Post("/GetCustomerAssessmentVendors", params);
        if (res.status === 2000) {
          this.list = res.data.list;
          this.list = res.data.list;
          let list = JSON.parse(JSON.stringify(res.data.list));
          this.getlie(list);
          console.log(this.list, "数据源");
