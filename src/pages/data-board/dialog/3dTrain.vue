...
<script>
	...
	export default {
		...
		methods: {
			...
			getPerList() {
				this.loading = true;
				this.Post("/GetTrainTaskTaskDetailDaPing", {
					page: 1,
					rows: 9999,
					TrainTypeCode: '0004503',
					IsThisMonth : 1,
			...
			init() {
				...
				this.table.forEach((item, i) => {
					...
					number.textContent = "任务名称:" + item.TrainTaskName || "";
					element.appendChild(number);

					var EnterpriseName = document.createElement("div");
					EnterpriseName.className = "EnterpriseName";
					EnterpriseName.title = "企业:" + item.EnterpriseName || "";
					EnterpriseName.textContent = "企业:" + item.DepartmentListJustShow.map(item => item.EnterpriseName).join('、') || "";
					element.appendChild(EnterpriseName);

					var Name = document.createElement("div");
					Name.className = "number";
					Name.textContent = "部门:" + item.DepartmentListJustShow.map(item => item.DepartmentName).join('、') || "";
					element.appendChild(Name);

					var Time = document.createElement("div");
					Time.className = "number";
					Time.textContent = item.TrainWay == 2 ? "完成率:" + item.CompleteRate + '%' : "完成率:" + '-'
