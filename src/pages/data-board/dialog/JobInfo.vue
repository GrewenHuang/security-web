...
<script>
	...
	export default {
		...
		methods: {
			...
			getPerList() {
				this.loading = true;
				this.Post("/GetOperationStandards", {
					page: 1,
					rows: 9999,
			...
			init() {
				...
				this.table.forEach((item, i) => {
					...
					number.textContent = "业务名称:" + item.BusinessName;
					element.appendChild(number);

					var EnterpriseName = document.createElement("a");
					EnterpriseName.className = "EnterpriseName";
					EnterpriseName.title = "作业名:" + item.OperationName;
					EnterpriseName.textContent = "作业名:" + item.OperationName || "";
					element.appendChild(EnterpriseName);

					var Name = document.createElement("div");
					Name.className = "number";
					Name.textContent = "作成人:" + item.BusinessName || "";
					element.appendChild(Name);

					var Time = document.createElement("div");
					Time.className = "number";
					Time.textContent = "作成时间:" + item.BedoneTimeDesc || "";
			...
			onClickDetail(event, i) {
				this.$refs.job.show(this.table[i], true, "")
			},
			render() {
				this.renderer.render(this.scene, this.camera);
			},
