	...
	export default {
		...
		methods: {
			...
			show(EnterpriseId,data,dep) {
				...
				this.isShowF = true
				this.$nextTick(() => {
					this.getPerList(EnterpriseId,data,dep)
				})
			},
			getPerList(EnterpriseId,data,dep) {
			...
			init() {
				...
				this.table.forEach((item, i) => {
					...
					cardImg.src = item.Photo ? item.Photo : _this.ImgHH
					element.appendChild(cardImg);

					var number = document.createElement("div");
					number.className = "number";
					number.textContent = "姓名:" + item.UserName;
					...
					EnterpriseName.textContent = "企业:" + item.EnterpriseName;
