            ...
            <div :style="{width: `calc(${single ? '100%' : '100% - 300px'})`}">
                <el-scrollbar class="scrollbar">
                    ...
                    >
                        ...
                        <el-table-column label="技术交底名称" prop="ProjectName">
                        </el-table-column>
...
export default {
    ...
    methods: {
        ...
        updateCheckAll() {
            let pageIds = this.list.map(item => item.TechnicalId);
            let currentPageChoosedTotal = this.choosed.filter(item => pageIds.includes(item.TechnicalId)).length;
        ...
        tableCheckboxChecked(row) {
            return this.choosed.some(item => item.TechnicalId == row.TechnicalId);
        },
        checkedItem(item, event ) {
            let index = this.choosed.map(item => item.TechnicalId).indexOf(item.TechnicalId);
            if (index == -1) {
        ...
        removeChoose(list) {
            if (list.length) {
                let removeIds = list.map(item => item.TechnicalId);
                this.choosed = this.choosed.filter(u => !removeIds.includes(u.TechnicalId));
            }
        },
