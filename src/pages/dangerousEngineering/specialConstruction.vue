            ...
            <div :style="{width: `calc(${single ? '100%' : '100% - 300px'})`}">
                <el-scrollbar class="scrollbar">
                    ...
                    >
                        ...
                        <el-table-column label="专项施工方案名称" prop="ProjectName">
                        </el-table-column>
...
export default {
    ...
    methods: {
        ...
        updateCheckAll() {
            let pageIds = this.list.map(item => item.SpecialItemId);
            let currentPageChoosedTotal = this.choosed.filter(item => pageIds.includes(item.SpecialItemId)).length;
        ...
        tableCheckboxChecked(row) {
            return this.choosed.some(item => item.SpecialItemId == row.SpecialItemId);
        },
        checkedItem(item, event ) {
            let index = this.choosed.map(item => item.SpecialItemId).indexOf(item.SpecialItemId);
            if (index == -1) {
        ...
        removeChoose(list) {
            if (list.length) {
                let removeIds = list.map(item => item.SpecialItemId);
                this.choosed = this.choosed.filter(u => !removeIds.includes(u.SpecialItemId));
            }
        },
        ...
        async getList() {
            ...
            this.loading = true;
            try {
                let res = await this.Post("/GetSpecialItems", params);
                if (res.status == 2000) {
