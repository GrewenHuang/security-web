        ...
        <div class="grid" ref="chart">
            <div class="card">
                ...
                <div class="card-content">
                    <pie :value="info.TypeProportions" :loading="loading" :labelFormat="labelFormat"/>
                </div>
            </div>
            <div class="card">
...
export default {
    ...
    methods: {
        exportPDF () {
            ...
            this.$nextTick(() => {
                Pdf.downPdf(this.$refs.chart, this.filters.year + '年作业统计').then(res => {
                    setTimeout(() => {
                        loading.close();
                    }, 1000);
                });
        ...
        async getData() {
            this.loading = true;
            let res = await this.Get(`/GetJobTicketReport?year=${this.filters.year}&isSubEnterprise=0`);
            if (res.status == 2000) {
