<template>
    <div class="chart" ref="chart" v-loading="loading">
        <div slot="header" class="custom-operation">
            ...
            <el-button type="primary" @click="exportPDF"  v-if="$_has('YHTJBtnExport')">导出</el-button>
        </div>
        <div class="grid">
            <div class="card">
...
export default {
    ...
    methods: {
        ...
        async getData() {
            this.loading = true;
            let res = await this.Get(`/GetPotentialReport?year=${this.filters.year}&isSubEnterprise=0`);
            if (res.status == 2000) {
