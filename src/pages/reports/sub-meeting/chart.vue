...
export default {
    ...
    methods: {
        ...
        async getData() {
            this.loading = true;
            let res = await this.Get(`/GetMeetingReport?year=${this.filters.year}&isSubEnterprise=1`);
            if (res.status == 2000) {
                ...
                this.info = res.data;
                if (!this.info.PreMeetingRate) {
                    this.info.PreMeetingRate = [];
                }
                if (!this.info.SafetyMeetingGroupRate) {
                    this.info.SafetyMeetingGroupRate = [];
                }
                if (!this.info.SafetyMeetingRate) {
                    this.info.SafetyMeetingRate = [];
                }
                this.loading = false;
            }
        }
