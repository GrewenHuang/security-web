<!-- <template>
	<tabs-page-layout :tabs="tabs" overflowInitial v-if="tabs && tabs.length > 0"></tabs-page-layout>
...
<template>
	<div style="position: relative;">
		<div slot="header" class="f-e-c">
			<div class="export-box">
				<el-button type="primary" @click="exportPDF" v-if="$_has('CLTJBtnExport')">导出</el-button>
			</div>
		</div>
		<div class="chart" ref="chart" :loading="loading">
			<div class="grid">
				<div class="card">
					<div class="title">企业车辆</div>
				...
				<div class="card">
					...
					<div class="card-content">
						<pie :value="info.StatusProportions" :color="['#5292FF', '#33D0BD', '#FFC372', '#FD706E']"
							:labelFormat="labelFormat" />
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				...
				// let res = await this.Get(`/GetVehicleReport?year=${Year}`);
				let res = await this.Get(`/GetVehicleReport`)
				if (res.status == 2000) {
