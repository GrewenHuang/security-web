<template>
    ...
    >
        ...
        <tr>
            ...
            <td>作业申请时间</td>
            <td colspan="3">{{ baseInfo.ApplyTimeDesc | format(dateFormatStr) }}</td>
        </tr>
        <!-- 
            specialFields
            * field1: 作业地点
        ...
        <tr>
            ...
            <td>{{ specialFields.field1 }}</td>
            <td>作业内容</td>
            <td colspan="3">{{ specialFields.field2 }}</td>
        </tr>
        <tr>
            <td colspan="2">作业高度</td>
            <td>{{ specialFields.field3 }}</td>
            <td>高处作业级别</td>
            <td colspan="3">{{ specialFields.field4 }}</td>
        </tr>
        <tr>
            <td colspan="2">作业单位</td>
            <td>{{ baseInfo.Company }}</td>
            <td>监护人</td>
            <td colspan="3">{{ getValueByName('监护人') }}</td>
        </tr>
        <tr>
            <td colspan="2">作业人</td>
        ...
        <tr>
            <td colspan="2">关联的其它特殊作业及<br/>安全作业票编号</td>
            <td colspan="5">{{ joinJob }}</td>
        </tr>
        <tr>
            <td colspan="2">风险辨识结果</td>
            <td colspan="5">{{ riskResult }}</td>
        </tr>
        <tr>
            <td colspan="2">作业实施时间</td>
            <td colspan="5">自 {{ baseInfo.JobTicketStartTimeDesc | format(dateFormatStr) }} 至 {{ baseInfo.JobTicketEndTimeDesc | format(dateFormatStr) }} 止</td>
        </tr>
        <tr>
            <td>序号</td>
        ...
        <template v-for="(item, index) in measureList">
            <tr :key="index">
                <td>{{ index + 1}}</td>
                <td colspan="4" style="text-align:left;">{{ item.Content }}</td>
            ...
            <td colspan="7">
                ...
                <div class="clearbox">
                    <div style="float: right;display: flex;align-items:flex-end;min-width: 200px;">
                        <div>签字：</div>
                        <div>
                            <template v-for="(sign, i) in getApprovalUserInfoByName('安全主管')">
                                <span :key="i">
                                    <img style="width: 60px;height: 30px;" :src="sign.SignInUrl" />
                                    <span style="margin-left: 10px;">{{ sign.ApprovalTimeDesc | format(dateFormatStr) }}</span>
                                    <br />
                    ...
                    <div style="padding-top: 17px;">
                        <template  v-for="(item, index) in getApprovalUserInfoByName('安全主管')">
                            <span :key="index">{{ item.ApprovalOpinion }}</span>
                        </template>
                    </div>
                </div>
