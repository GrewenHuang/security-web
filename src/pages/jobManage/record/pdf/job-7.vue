<template>
    ...
    >
        ...
        <tr>
            <!-- * field1: 作业地点
             * field2: 作业内容
             * field3: 作业范围/内容/方式（包括深度/面积，并附见图）
             * field4: 附件图
             * field5： 编制人签字
             * field6： 编制人签字时间 -->
        ...
        <tr>
            <td colspan="2">关联的其他特殊作业<br/>及安全作业票编号</td>
            <td colspan="9">{{ joinJob }}</td>
        </tr>
        <tr>
            <td colspan="11">
                <div style="text-align: left;">作业范围、内容、方式（包括深度、面积，并附见图）：</div>
        ...
        <tr>
            ...
            <td colspan="9">{{ riskResult }}</td>
        </tr>
        <tr>
            <td colspan="2">作业实施时间</td>
            <td colspan="9">自 {{ baseInfo.JobTicketStartTimeDesc | format(dateFormatStr) }} 至 {{ baseInfo.JobTicketEndTimeDesc | format(dateFormatStr) }} 止</td>
        </tr>
        <tr>
            <td>序号</td>
        ...
        <template v-for="(item, index) in measureList">
            <tr :key="index">
                <td>{{ index + 1}}</td>
                <td colspan="7" style="text-align:left;">{{ item.Content }}</td>
        ...
        <tr style="text-align:left;">
            <td colspan="11">
                <div>有关水、电、汽、工艺、设备、消防、安全等部门会签意见</div>
                <div class="clearbox">
                    <div style="float: right;display: flex;align-items:flex-end;min-width: 200px;">
                        <div>签字：</div>
                        <div>
                            <template v-for="(sign, i) in getApprovalUserInfoByName('水、电、汽、工艺、设备、消防、安全等部门会签')">
                                <span :key="i">
                                    <img style="width: 60px;height: 30px;" :src="sign.SignInUrl" />
                                    <span style="margin-left: 10px;">{{ sign.ApprovalTimeDesc | format(dateFormatStr) }}</span>
                                    <br />
                    ...
                    <div style="padding-top: 17px;">
                        <template  v-for="(item, index) in getApprovalUserInfoByName('水、电、汽、工艺、设备、消防、安全等部门会签')">
                            <span :key="index">{{ item.ApprovalOpinion }}</span>
                        </template>
                    </div>
                </div>
