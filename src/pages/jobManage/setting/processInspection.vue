<template>
    <div class="page-content">
        <div class="filter-content">
            <div class="f-b-c">
                <div class="operation">
                    <!-- <el-input placeholder="监督表名称" v-model="filters.Name"></el-input>
...
import info from './processInspectionInfo';
export default {
    components: {
        info
    },
    ...
    methods: {
        ...
        async getList() {
            ...
            this.loading = true;
            try {
                let res = await this.Get('/GetJobProcessCheckList', params);
                if (res.status === 2000) {
                    this.list = res.data;
        ...
        remove(row) {
            this.$confirm('确认删除该条数据吗？').then((_) => {
                this.Get('/DeleteJobProcessCheck', {
                    jobProcessCheckId: row.JobProcessCheckId,
                }).then((res) => {
                    if (res.status == 2000) {
                        this.getList();
                        this.$message({
