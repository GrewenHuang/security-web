<template>
    ...
    >
        <el-form style="margin-top: 10px;" ref="form" label-position="top" :model="info" :rules="rules">
            <el-form-item label="常规作业类型" prop="Name">
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                Name: [{required: true, message: '请输入常规作业类型', trigger: ['change', 'blur']}],
            }
        }
    },
    computed: {
            ...
            this.$refs.form.validate(async (valid, fields) => {
                if (valid) {
                   ...
                   try {
                      ...
                      let res = await this.Post('/SaveJobTemplate', params)
                      if (res.status == 2000) {
