    ...
    >
        <el-scrollbar ref="scrollbar" class="scrollbar">
            <el-form ref="form" label-position="top" :model="info" :rules="rules">
                <div class="f-b-c">
                    <el-form-item class="w50 mr20" label="名称" prop="Name">
                        <el-input placeholder="请输入" v-model="info.Name" ></el-input>
                    </el-form-item>
                    <el-form-item class="w50" label="事故类型" prop="CategoryCode">
                ...
                <div class="f-b-c">
                    <el-form-item class="w50" label="事故等级" prop="AccidentLevelCode">
                        <el-select placeholder="请选择" v-model="info.AccidentLevelCode" @change="handleChange('AccidentLevelName', 'AccidentLevelCode', 'levelList')">
                            <el-option v-for="(item, index) in levelList" :key="index" :label="item.text" :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item class="w50" label="事故发生日期" prop="ImplementTime">
                        <el-date-picker type="date" value-format="yyyy-MM-dd" placeholder="请选择" v-model="info.ImplementTime" ></el-date-picker>
                    </el-form-item>
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                CategoryCode: [{required: true, message: '请选择事故类型', trigger: ['change', 'blur']}],
                AccidentLevelCode: [{required: true, message: '请选择事故等级', trigger: ['change', 'blur']}],
                ImplementTime: [{required: true, message: '请选择事故发生日期', trigger: ['change', 'blur']}],
            },
            typeList: [],
            levelList: [],
        }
    ...
    methods: {
        async show(item) {
            ...
            this.typeList = await this.getDictionaryById('200004');
            this.levelList = await this.getDictionaryById('00019');
            if (item) {
                this.loading = true;
