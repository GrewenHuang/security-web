            ...
            >
                <template slot-scope="scope">
                    <div class="table-operation">
                        <el-button
                            type="text"
                            @click="cancelNum(scope.row.PotentialId)"
                            >申请销号</el-button
                        >
                        <el-button
                            type="text"
...
export default {
    ...
    methods: {
        ...
        cancelNum(PotentialId) {
            this.$confirm("您是否确定销号?", "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning",
                center: true,
            }).then(() => {
                this.Post("SaveEnterprisePotentialApplyCancel", {
                    PotentialId,
                }).then((res) => {
                    if (res.status == 2000) {
                        this.getList();
        ...
        getList() {
            ...
            this.loading = true;
            this.Post("/GetEnterprisePotentialWaitForApplyCancel", params)
                .then((res) => {
                    if (res.status == 2000) {
