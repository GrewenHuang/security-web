  ...
  >
    ...
    <div slot="operation" class="f-b-c">
      <div class="input-content">
        <el-form :model="filters" :inline="true">
          ...
          <el-form-item>
            ...
            <el-button type="primary" @click="excelImport">导出</el-button>
            <el-popover width="300" trigger="click">
              <el-form-item>
                <el-input
                  v-model="filters.RectificationUserName"
                  clearable
                  placeholder="整改人"
                ></el-input>
              </el-form-item>
              <el-form-item>
                <el-input
                  v-model="filters.ReexamineUserName"
                  clearable
                  placeholder="复查人"
                ></el-input>
              </el-form-item>
      ...
      <div class="f-c-c">
        <el-button-group>
          <el-button
            @click="downloadmbs(1510, 15102, filters)"
            v-if="$_has('YHZLBtnPCTZ') || $_has('RWXQYHBTNPCTZ')"
            >隐患排查台账</el-button
          >
          <el-button
            @click="downloadmbs(1510, 15103, filters)"
            v-if="$_has('YHZLBtnZLTZ') || $_has('RWXQYHBTNZLTZ')"
            >隐患治理台账</el-button
          >
        </el-button-group>
      </div>
    ...
    >
      ...
      <el-table-column label="责任部门" prop="DepartmentName" align="center">
      </el-table-column>
      <el-table-column
        label="上报人"
        prop="SubmitUserName"
      ...
      >
        <template slot-scope="{ row }">
          <span v-if="row.ResponsibleName">{{ row.ResponsibleName }}</span>
          <span v-else>{{ row.NotifyDepartmentHeadName }}</span>
          <!-- <span
						v-if="scope.row.RecordRectification && scope.row.RecordRectification.length > 0">{{ scope.row.RecordRectification.map(item => item.OperationUserName).join('、') }}</span>
          ...
          <div class="table-operation">
            <el-popover placement="bottom" width="250" trigger="click">
              ...
              <el-button
                slot="reference"
                type="text"
                v-if="$_has('YHZLBtnDown') || $_has('RWXQYHBTNDOWN')"
                >下载</el-button
              >
            </el-popover>
            <el-button
              type="text"
              @click="showViewDialog(scope.row.PotentialId, scope.row)"
              v-if="$_has('YHZLBtnLook') || $_has('RWXQYHBTNLOOK')"
              >查看</el-button
            >
          ...
          <div>
            <el-form ref="form">
              ...
              <div class="form-col">
                ...
                <div>
                  安全等级：{{
                    row.SecurityLevelDesc ? row.SecurityLevelDesc : "0"
                  }}
                </div>
              </div>
              <div class="form-col">
              ...
              <el-card class="box-card">
                <el-form-item label="排查照片：">
                  <div>
                    <mediaList
                      :list="row.RecordSubmit.AttachmentFiles"
              ...
              <template v-if="row.RecordRectification">
                <div
                  v-for="(item, index) in row.RecordRectification"
                  :key="index"
                >
                  <el-row>
                    <el-card class="box-card">
              ...
              <template v-if="row.RecordReexamine">
                <div
                  v-for="(data, ids) in row.RecordReexamine"
                  :key="ids + 'a'"
                >
                  <el-row>
                    <el-card class="box-card">
...
export default {
  ...
  methods: {
    ...
    excelImport() {
      let workbook = XLSX.utils.table_to_book(
        document.getElementById("outTable")
      ); //需要在table上定义一个id
      // 可以打印出来看看workbook--中有那些属性
      workbook.Sheets.Sheet1["!cols"][11] = { hidden: true };
      try {
        XLSX.writeFile(workbook, "隐患治理.xlsx");
        console.log("成功");
      } catch (e) {
    ...
    getList: function() {
      ...
      that.loading = true;
      this.Post("/GetEnterprisePotentialRecords", params)
        .then(res => {
          if (res.status == 2000) {
  ...
  mounted() {
    ...
    //获取隐患排查性质
    this.Get("/GetPotentialSourceCom").then(res => {
      if (res.status === 2000) {
