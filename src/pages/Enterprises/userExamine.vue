<template>
	<page-layout :loading="loading" :pages="pages" @current-change="handlePageChange">
		<div slot="operation" class="f-b-c">
			<div class="input-content">
				<el-form :inline="true" :model="filters">
					...
					<el-form-item>
						<cascader-department placeholder="部门" clearable
							v-model="filters.DepartmentId" :enterpriseId="filters.EnterpriseId"
					...
					<el-form-item>
						<el-button type="primary" @click="batchExamine(1)" v-if="$_has('RYSHBtnPLTG')">批量通过</el-button>
					</el-form-item>
					<el-form-item>
						<el-button type="danger" @click="batchExamine(-1)" v-if="$_has('RYSHBtnPLBH')">批量驳回</el-button>
					</el-form-item>
		...
		<el-table :data="list" highlight-current-row :header-cell-style="$tableStyle" style="width: 100%"
			...
			<el-table-column type="selection" width="55" align="center" v-if="filters.ApproveStatus == 0">
			</el-table-column>
			<el-table-column label="序号" type="index" align="center" width="50">
			</el-table-column>
			<!-- <el-table-column label="照片" align="center" prop="image">
			</el-table-column> -->
			...
			<el-table-column label="操作" align="center" width="120">
				<template slot-scope="{ row }">
					<div class="table-operation">
						<el-button type="text" @click="add(row)" v-if="$_has('RYSHBtnSH') || $_has('RYSHBtnLook')">{{ row.ApproveStatus == 0 ? '审核' : '查看'}}</el-button>
						<el-button type="text" size="mini" @click="reject(row)" v-if="$_has('RYSHBtnBH')">驳回</el-button>
					</div>
				</template>
			</el-table-column>
		</el-table>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				...
				this.loading = true;
				try {
					let res = await this.Post('/GetEnterpriseUsersByApprove', params);
					if (res.status === 2000) {
			...
			async batchExamine(ApproveStatus) {
