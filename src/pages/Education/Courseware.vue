    ...
    >
        ...
        <div slot="operation" class="f-b-c">
            <div class="input-content">
                <el-input
                    v-model="filters.CoursewareName"
                    placeholder="课件名称"
            ...
            >
                <template slot-scope="scope">
                    <div class="table-operation">
                        <el-button type="text" @click="editCour(scope.row)"
                            >编辑</el-button
                        >
                        <el-button
                            type="text"
                            text-plain
                            @click="delCour(scope.row.CoursewareId)"
                            >删除</el-button
                        >
                    </div>
                </template>
                    ...
                    <div>
                        ...
                        >
                            ...
                            <el-table-column label="培训时长(秒)" width="110">
                                <template slot-scope="scope">
                                    <el-input
                                        v-model="scope.row.Lengths"
                                        :disabled="
                                            /mp4|mp3/.test(scope.row.Ext)
                                        "
                                    ></el-input>
                                </template>
                            </el-table-column>
                        ...
                        >
                            <el-upload
                                action=""
                                accept=".mp4,.mp3"
                                :http-request="handleAvatarSuccessMP4"
                                :before-upload="beforeAvatarUploadMP4"
                                :show-file-list="false"
                                v-if="uploadModel == 1"
                            >
                                <el-button size="small" type="primary" style="margin-right: 9px;"
                                    >上传音视频</el-button
                                >
                            </el-upload>
                            <custom-upload
                            ...
                            <el-upload
                                action=""
                                accept=".pdf"
                                :http-request="handleAvatarSuccessPDF"
    ...
    methods: {
        ...
        handleAvatarSuccessPDF(e) {
            ...
            that.Get("/GetOssToken", {}).then((result) => {
                if (result.status == 2000) {
                    ...
                    let putExtra = {
                        params: {}, //用来放置自定义变量
                        mimeType: ["application/pdf"], //用来限制上传文件类型，为 null 时表示不对文件类型限制；限制类型放到数组里： ["image/png", "image/jpeg", "image/gif"]
                    };
        ...
        handleAvatarSuccessMP4(e) {
            ...
            that.Get("/GetOssToken", {}).then((result) => {
                if (result.status == 2000) {
                    ...
                    let putExtra = {
                        params: {}, //用来放置自定义变量
                        mimeType: ["video/mp4"], //用来限制上传文件类型，为 null 时表示不对文件类型限制；限制类型放到数组里： ["image/png", "image/jpeg", "image/gif"]
                    };
                    var observable = qiniu.upload(
                        e.file,
                        key,
        ...
        async preview(url, item) {
            if (url.indexOf("http") == -1) {
                let result = await this.Get("/GetOssUrl", { key: url });
                url = result.data.url;
            }
