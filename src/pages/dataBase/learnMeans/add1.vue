        ...
        <el-scrollbar ref="scrollbar" class="scrollbar">
            <el-form ref="form" label-position="top" :model="info" :rules="rules">
                <div class="f-b-c">
                    ...
                    <el-form-item class="w50" label="发文机关" prop="ReleaseAuthority">
                        <el-input placeholder="请输入" v-model="info.ReleaseAuthority" ></el-input>
                    </el-form-item>
                </div>
                <div class="f-b-c">
                    <el-form-item class="w50 mr20" label="文号" prop="FileNo">
                        <el-input placeholder="请输入" v-model="info.FileNo" ></el-input>
                    </el-form-item>
                    <el-form-item class="w50" label="实施日期" prop="ImplementTime">
                        <el-date-picker type="date" value-format="yyyy-MM-dd" placeholder="请选择" v-model="info.ImplementTime" ></el-date-picker>
                    </el-form-item>
                </div>
                <div class="f-b-c">
                    <el-form-item class="w50 mr20" label="分类" prop="CategoryCode">
                        <el-select placeholder="请选择" v-model="info.CategoryCode" @change="changeType" >
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                CategoryCode: [{required: true, message: '请选择分类', trigger: ['change', 'blur']}],
                ImplementTime: [{required: true, message: '请选择实施日期', trigger: ['change', 'blur']}],
            },
        }
    },
    async mounted() {
    ...
    methods: {
        async show(item) {
            ...
            this.typeList = await this.getDictionaryById('200003');
            if (item) {
                this.loading = true;
        ...
        changeType() {
            this.info.CategoryName = this.typeList.filter(item => item.id == this.info.CategoryCode)[0].text;
        }
    }
}
</script>
