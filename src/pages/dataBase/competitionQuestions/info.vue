<template>
	<el-dialog v-dialog-drag :title="title" :visible.sync="dislogShow" @close="close()" class="info" width="1000px"
		...
		<el-scrollbar ref="scrollbar" class="scrollbar">
			<el-form ref="form" label-position="top" :model="info" :rules="rules" v-if="!isPreview">
				...
				<div class="form-2-col">
					<el-form-item label="试题类型" prop="ExamQuestionTypeId">
						<el-select placeholder="请选择类型" v-model="info.ExamQuestionTypeId" clearable style="width: 100%">
				...
				<el-form-item prop="ContestAnswers" ref="ContestAnswers">
					...
					<el-table :data="info.ContestAnswers" :header-cell-style="$tableStyle">
						...
						<el-table-column label="正确选项" align="center" width="120">
							<template slot-scope="{ row }">
								<el-checkbox :true-label="1" :false-label="0" v-model="row.IsCorrect"
									@change="answerCorrectChange(row)"></el-checkbox>
							</template>
						</el-table-column>
						<el-table-column label="操作" align="center" width="90">
							<template slot-scope="{ $index }">
								<el-button type="text" text-plain v-if="$index < 2">删除</el-button>
								<el-button type="text" @click="info.ContestAnswers.splice($index, 1)"
									v-else>删除</el-button>
							</template>
						</el-table-column>
					</el-table>
			...
			<el-form v-else>
				...
				<el-table :data="info.ContestAnswers" :header-cell-style="$tableStyle">
					...
					<el-table-column label="内容" prop="Answer">
					</el-table-column>
					<el-table-column label="正确选项" align="center" width="120">
						<template slot-scope="{ row }">
							<el-checkbox :true-label="1" :false-label="0" v-model="row.IsCorrect"
		...
		<div slot="footer" class="dialog-footer">
			...
			<el-button @click.native="close()" v-else>关闭</el-button>
		</div>
	</el-dialog>
</template>
		...
		methods: {
				...
				this.$refs.form.validate(async (valid, fields) => {
					...
					if (valid) {
						if (this.info.ContestAnswers.every(item => item.IsCorrect == 0)) {
							...
							this.$refs.scrollbar.wrap.scrollTop = this.$refs['ContestAnswers'].$el.offsetTop;
							return;
						} else if (this.info.TypeName == '多选题' && this.info.ContestAnswers.filter(item => item
								.IsCorrect).length < 2) {
			...
			typeChange() {
				...
				if (this.info.TypeName == '判断题') {
					...
					this.info.ContestAnswers = [_extends(defaultItem), _extends(defaultItem), _extends(defaultItem),
						_extends(defaultItem)
					];
				}
			},
