<template>
    ...
    >
        ...
        <div class="f-b-c">
            <div v-if="!single">
                已选：{{ choosed[0] ? choosed[0].Name : '未选择岗位' }}
            </div>
            <!--分页 开始-->
            <el-pagination
        ...
        <div slot="footer">
            ...
            <el-tooltip :disabled="!!choosed.length" placement="top" content="请选择岗位">
...
export default {
    ...
    methods: {
        ...
        updateCheckAll() {
            let pageIds = this.list.map(item => item.PositionId);
            let currentPageChoosedTotal = this.choosed.filter(item => pageIds.includes(item.PositionId)).length;
        ...
        tableCheckboxChecked(row) {
            return this.choosed.some(item => item.PositionId == row.PositionId);
        },
        checkedItem(item, event ) {
            let index = this.choosed.map(item => item.PositionId).indexOf(item.PositionId);
            if (index == -1) {
        ...
        removeChoose(list) {
            if (list.length) {
                let removeIds = list.map(item => item.PositionId);
                this.choosed = this.choosed.filter(u => !removeIds.includes(u.PositionId));
            }
        },
        ...
        async getList() {
            ...
            this.loading = true;
            try {
                let res = await this.Post("/GetPositionList", params);
                if (res.status == 2000) {
