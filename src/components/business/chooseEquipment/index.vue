<template>
    ...
    >
            ...
            <div :style="{width: `calc(${single ? '100%' : '100% - 300px'})`}">
                <el-scrollbar class="scrollbar">
                    ...
                    >
                        ...
                        <el-table-column label="设备类型" prop="EquipmentTypeName">
                        </el-table-column>
        ...
        <div class="f-b-c">
            <div v-if="single">
                已选：{{ choosed[0] ? choosed[0].EquipmentName : '未选择设备' }}
            </div>
            <!--分页 开始-->
            <el-pagination
        ...
        <div slot="footer">
            ...
            <el-tooltip :disabled="!!choosed.length" placement="top" content="请选择设备">
...
export default {
    ...
    methods: {
        ...
        updateCheckAll() {
            let pageIds = this.list.map(item => item.EquipmentManageId);
            let currentPageChoosedTotal = this.choosed.filter(item => pageIds.includes(item.EquipmentManageId)).length;
        ...
        tableCheckboxChecked(row) {
            return this.choosed.some(item => item.EquipmentManageId == row.EquipmentManageId);
        },
        checkedItem(item, event ) {
            let index = this.choosed.map(item => item.EquipmentManageId).indexOf(item.EquipmentManageId);
            if (index == -1) {
        ...
        removeChoose(list) {
            if (list.length) {
                let removeIds = list.map(item => item.EquipmentManageId);
                this.choosed = this.choosed.filter(u => !removeIds.includes(u.EquipmentManageId));
            }
        },
