<template>
    ...
    <div class="f-b">
            ...
            <div :style="{width: `calc(${single ? '100%' : '100% - 300px'})`}">
                <el-scrollbar class="scrollbar">
                    ...
                    >
                        ...
                        <el-table-column label="专项施工方案名称" prop="Name">
                        </el-table-column>
                    </el-table>
                    <el-table
                        v-else
        ...
        <div class="f-b-c">
            <div v-if="single">
                已选：{{ choosed[0] ? choosed[0].Name : '未选择方案' }}
            </div>
            <!--分页 开始-->
            <el-pagination
        ...
        <div slot="footer">
            ...
            <el-tooltip :disabled="!!choosed.length" placement="top" content="请选择方案">
...
export default {
    ...
    methods: {
        ...
        updateCheckAll() {
            let pageIds = this.list.map(item => item.SpecialItemDatabaseId);
            let currentPageChoosedTotal = this.choosed.filter(item => pageIds.includes(item.SpecialItemDatabaseId)).length;
        ...
        tableCheckboxChecked(row) {
            return this.choosed.some(item => item.SpecialItemDatabaseId == row.SpecialItemDatabaseId);
        },
        checkedItem(item, event ) {
            let index = this.choosed.map(item => item.SpecialItemDatabaseId).indexOf(item.SpecialItemDatabaseId);
            if (index == -1) {
        ...
        removeChoose(list) {
            if (list.length) {
                let removeIds = list.map(item => item.SpecialItemDatabaseId);
                this.choosed = this.choosed.filter(u => !removeIds.includes(u.SpecialItemDatabaseId));
            }
        },
        ...
        async getList() {
            ...
            this.loading = true;
            try {
                let res = await this.Post("/GetSpecialItemDatabases", params);
                if (res.status == 2000) {
