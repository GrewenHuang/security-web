...
export default {
    ...
    methods: {
        ...
        updateCheckAll() {
            let pageIds = this.list.map(item => item.EquipmentId);
            let currentPageChoosedTotal = this.choosed.filter(item => pageIds.includes(item.EquipmentId)).length;
        ...
        tableCheckboxChecked(row) {
            return this.choosed.some(item => item.EquipmentId == row.EquipmentId);
        },
        checkedItem(item, event ) {
            let index = this.choosed.map(item => item.EquipmentId).indexOf(item.EquipmentId);
            if (index == -1) {
        ...
        removeChoose(list) {
            if (list.length) {
                let removeIds = list.map(item => item.EquipmentId);
                this.choosed = this.choosed.filter(u => !removeIds.includes(u.EquipmentId));
            }
        },
