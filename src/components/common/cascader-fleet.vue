<template>
	<el-cascader class="select-fleet" v-model="selected" :filterable="multiple || filterable"
		...
		:props="{ value: 'VehicleGroupSetId', label: 'Name', children: 'children',emitPath: false,checkStrictly: true  }">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getUsers() {
				try {
					let res = await this.Get('/GetTreeVehicleGroupSets');
					if (res.status == 2000) {
						if (res.data) {
							this.list = res.data;
